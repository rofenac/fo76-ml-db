#!/usr/bin/env python3
"""
Fallout 76 Wiki Power Armor Scraper

Scrapes power armor data from Fallout Wiki pages and outputs clean CSV data
...
@dataclass
class PowerArmorData:
    """Structured power armor data matching database schema"""
    name: str
    type: str = ""  # Frame, Torso, Left Arm, Right Arm, Left Leg, Right Leg, Helmet
    set_name: str = ""  # T-45, T-51, T-60, X-01, etc.
    armor_rating: str = ""
    ...
    value: str = ""
    durability: str = ""
    fusion_core_drain: str = ""  # AP drain rate for frames
    form_id: str = ""
    editor_id: str = ""
    perks_raw: str = ""
    source_url: str = ""
    ...
    def to_csv_dict(self) -> Dict[str, str]:
        ...
        csv_data = {
            ...
            'Fusion Core Drain': data['fusion_core_drain'],
...
class FalloutPowerArmorScraper:
    """Scraper for Fallout Wiki power armor pages"""
    ...
    def scrape_power_armor(self, url: str, use_playwright: bool = False) -> List[PowerArmorData]:
        """
        Scrape power armor data from a Fallout Wiki URL

        Returns list of PowerArmorData objects (one per piece: frame, torso, helmet, arms, legs)
        """
        ...
        try:
            ...
            # Extract power armor set name
            set_name = self._extract_name(soup, url)

            # Extract power armor pieces from the page
            pa_pieces = self._extract_pa_pieces(soup, set_name, url)

            if pa_pieces:
                logger.info(f"Successfully scraped {len(pa_pieces)} pieces for {set_name}")
            else:
                logger.warning(f"No power armor pieces found for {set_name}")
    ...
    def _extract_name(self, soup: BeautifulSoup, url: str) -> str:
        """Extract power armor set name from page"""
    ...
    def _extract_pa_pieces(self, soup: BeautifulSoup, set_name: str, url: str) -> List[PowerArmorData]:
        """Extract individual power armor pieces from the page"""
        pieces = []
        ...
        # Power armor pieces: Frame, Helmet, Torso, Left Arm, Right Arm, Left Leg, Right Leg
        piece_types = ['Helmet', 'Torso', 'Left Arm', 'Right Arm', 'Left Leg', 'Right Leg']

        # Create entries for each piece
        # Note: For power armor, we typically create one entry per piece type
        for piece_type in piece_types:
            piece = PowerArmorData(
                name=f"{set_name} {piece_type.lower()}",
                type=piece_type,
                set_name=set_name,
                ...
                durability=infobox_data.get('durability', ''),
                fusion_core_drain=infobox_data.get('fusion_core_drain', ''),
    ...
    def _extract_infobox(self, infobox: BeautifulSoup) -> Dict[str, str]:
        """Extract data from the power armor infobox"""
        data = {}
        ...
        for row in rows:
            ...
            if 'armor' in label and ('rating' in label or 'dr' in label):
                data['armor_rating'] = value
            elif 'physical' in label and 'resistance' in label:
                data['armor_rating'] = value
                ...
                data['value'] = value
            elif 'durability' in label or 'hp' in label:
                data['durability'] = value
            elif 'fusion' in label or 'drain' in label or 'ap' in label:
                data['fusion_core_drain'] = value
    ...
    def _extract_perks(self, soup: BeautifulSoup) -> List[Dict[str, any]]:
        """Extract perks that affect this power armor"""
        perks = []
    ...
    def scrape_urls_from_file(self, urls_file: str, output_csv: str, use_playwright: bool = False):
        """Scrape multiple power armor URLs and save to CSV"""
        # Read URLs
        ...
        for i, url in enumerate(urls, 1):
            ...
            pieces = self.scrape_power_armor(url, use_playwright=use_playwright)
            all_pa_pieces.extend(pieces)

        # Save to CSV
        if all_pa_pieces:
            self._save_to_csv(all_pa_pieces, output_csv)
            logger.info(f"Saved {len(all_pa_pieces)} power armor pieces to {output_csv}")
        else:
            logger.error("No power armor pieces scraped successfully")

    def _save_to_csv(self, pa_pieces: List[PowerArmorData], output_file: str):
        """Save power armor data to CSV"""
        fieldnames = ['Name', 'Type', 'Set Name', 'Armor Rating', 'Energy Resistance',
                     'Radiation Resistance', 'Level', 'Weight', 'Value', 'Durability',
                     'Fusion Core Drain', 'Form ID', 'Editor ID', 'Perks', 'Source URL']
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Scrape Fallout 76 power armor data from Wiki')
    parser.add_argument('-u', '--url', help='Single URL to scrape')
    parser.add_argument('-f', '--file', default='power_armor_urls.txt', help='File with URLs to scrape')
    parser.add_argument('-o', '--output', default='data/input/power_armor.csv', help='Output CSV file')
    ...
    scraper = FalloutPowerArmorScraper(perks_csv_path=args.perks)

    if args.url:
        # Scrape single URL
        pieces = scraper.scrape_power_armor(args.url, use_playwright=args.playwright)
        if pieces:
            scraper._save_to_csv(pieces, args.output)
            print(f"\nScraped {len(pieces)} power armor pieces")
