#!/usr/bin/env python3
"""
Import consumables from multiple CSV files into NORMALIZED MySQL database
...
# File paths
CSV_FILES = [
    'data/input/Food_scraped.csv',
    'data/input/Soup_scraped.csv',
    'data/input/Drink_scraped.csv',
    'data/input/Chem_scraped.csv'
]
...
def parse_duration(duration_str: str) -> int:
    ...
    # Look for patterns like "1 hour", "30 minutes", "60 seconds", "90m"
    match = re.search(r'(\d+)\s*(hour|minute|second|min|sec|hr|m|h)', duration_str, re.IGNORECASE)
    if match:
...
def parse_numeric(value_str: str) -> float:
    ...
    # Handle values like "+5 Rads" or "-300"
...
def import_consumables_from_files(conn, caches):
    """Import consumables from multiple CSV files"""
    cursor = conn.cursor()
    ...
    for csv_file in CSV_FILES:
        ...
        with open(csv_file, 'r', encoding='utf-8') as f:
            ...
            print(f"  Loaded {len(consumables)} items from {csv_file}")

    print(f"\nTotal consumables to import: {len(all_consumables)}")
    ...
    for consumable in all_consumables:
        try:
            ...
            weight = parse_numeric(consumable.get('weight', ''))
            value = parse_numeric(consumable.get('value', ''))
    ...
    print(f"✓ Successfully imported {imported_count}/{len(all_consumables)} consumables")
...
def verify_import(conn):
    ...
    cursor.execute("SELECT COUNT(*) as count FROM consumables WHERE category IN ('alcohol', 'drink', 'beverage')")
    result = cursor.fetchone()
    drink_count = result['count']
    ...
    cursor.execute("SELECT COUNT(*) as count FROM consumable_effects")
    result = cursor.fetchone()
    ...
    print(f"  - Drinks/Alcohol: {drink_count}")
...
def main():
    print("="*60)
    print("CONSUMABLE MULTI-FILE IMPORT (NORMALIZED SCHEMA)")
    print("="*60)
    ...
    try:
        ...
        # Import consumables
        print("\nLoading CSV files...")
        imported = import_consumables_from_files(conn, caches)
